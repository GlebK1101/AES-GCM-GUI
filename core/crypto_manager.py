...
def random_filename(min_len: int = 16, max_len: int = 32) -> str: 
    if min_len < 1 or max_len < min_len:
        raise ValueError("Invalid name length boundaries")
...
def encrypt_file(input_path: str, password: str, enc_ext: str = ".enc", config=None, output_dir=None, custom_name=None):
    ...
    if len(ext_bytes) > 255:
        raise ValueError("Extension is too long")
    ...
    if output_dir and str(output_dir).strip():
        ...
        if not os.path.exists(target_dir):
            raise FileNotFoundError(f"Specified save folder does not exist: {target_dir}")
    else:
    ...
    if custom_name and str(custom_name).strip():
        ...
        if os.path.exists(output_path):
            ...
            raise FileExistsError(f"File named '{final_name + enc_ext}' already exists in target folder!") 
    else:
...
def encrypt_file_stream(input_path: str, vault_dir: str, password: str, enc_ext: str = ".enc", config=None):
    ...
    if len(name_bytes) > 65535: raise ValueError("Filename is too long")
    if len(ext_bytes) > 255: raise ValueError("Extension is too long")
        ...
        try:
            while True:
                ...
                if len(ciphertext) != chunk_size + 16:
                    raise ValueError("Container corrupted (incomplete chunk)")
...
def build_manifest(root_dir: str, vault_dir: str, password: str, exclude_dirs=None, config=None, status_callback=None, custom_manifest_dir=None):
    ...
    if not can_proceed:
        ...
        raise OSError(f"Insufficient space! Needed: {needed_mb} MB, available: {avail_mb} MB")
    
    files_to_encrypt = collect_files(root_dir, exclude_dirs)
    if not files_to_encrypt:
        raise ValueError("No files to encrypt (folder is empty or all files are excluded).")
    ...
    if os.path.exists(manifest_path):
        raise FileExistsError(f"Manifest file already exists at:\n{manifest_path}\nDelete it or choose another folder.")
    ...
    with open(manifest_path, "x", encoding="utf-8") as f_manifest:
        ...
        for i, rel_path in enumerate(files_to_encrypt):
            ...
            try:
                if status_callback: 
                    status_callback(f"[{i+1}/{len(files_to_encrypt)}] Encrypting: {rel_path}")
...
def restore_from_manifest(manifest_path: str, vault_dir: str, output_dir: str, password: str, config=None, status_callback=None):
    ...
    with open(manifest_path, "r", encoding="utf-8") as f:
        try:
            ...
            raise ValueError(f"Critical error reading JSON: {e}")
    ...
    for i, entry in enumerate(manifest):
        ...
        try:
            ...
            if common != abs_output_dir:
                msg = f"SECURITY ERROR: Path '{rel_path}' attempts to traverse outside restoration folder!"
            ...
            if status_callback: status_callback(f"Suspicious path (different drives): {rel_path}", is_error=True)
            continue
        # -------------------------------
        
        # Check if encrypted file exists
        if not os.path.exists(input_file):
            if status_callback: status_callback(f"File not found in vault: {stored_name}", is_error=True)
            continue
        ...
        try:
            ...
            if status_callback:
                status_callback(f"[{i+1}/{total}] Successfully restored: {rel_path}")
        
        except InvalidTag:
            # If password didn't work for one file, it won't work for others.
            err_msg = "INVALID PASSWORD! Restoration aborted."
            ...
            if status_callback: 
                status_callback(f"Error restoring {rel_path}: {e}", is_error=True)
...
def encrypt_file_stream_single(input_path: str, password: str, enc_ext: str = ".enc", config=None, output_dir=None, custom_name=None):
    ...
    if output_dir and str(output_dir).strip():
        target_dir = output_dir
        if not os.path.exists(target_dir):
            raise FileNotFoundError(f"Folder does not exist: {target_dir}")
    else:
        target_dir = os.path.dirname(input_path)
    ...
    if custom_name and str(custom_name).strip():
        ...
        output_path = os.path.join(target_dir, final_name + enc_ext)
        if os.path.exists(output_path):
            raise FileExistsError(f"File already exists: {final_name + enc_ext}")
    else:
        fname_params = config.get('filename_params', {})
        p_min_len = fname_params.get('min_len', 16)
        p_max_len = fname_params.get('max_len', 32)
    ...
    try:
        ...
        if free_space < estimated_total_size:
            ...
            raise OSError(f"Insufficient space (considering Chunk Size)!\nNeeded: {needed_mb:.2f} MB\nAvailable: {avail_mb:.2f} MB")
    ...
    with open(input_path, "rb") as fin:
        ...
        try:
            while True:
                ...
                ciphertext = fin.read(chunk_size + 16)
                if len(ciphertext) != chunk_size + 16:
                    raise ValueError("Container damaged or incorrect chunk size")
...
def generate_strong_password(length: int = 20, use_digits=True, use_symbols=True) -> str:
    ...
    """
    # I know that we have limited the minimum value to 8 with the slider, but you never know if you decide to change this minimum.
    if length < 4:
