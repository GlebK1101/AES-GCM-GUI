...
try:
    ...
    print("Ошибка импорта ядра в manifest.py")
...
class ManifestTab(BaseTab):
    def setup_ui(self):
        ...
        # Password Checkbox
        tk.Checkbutton(pad, text="Скрывать пароль", variable=self.is_password_hidden, 
        ...
        f_backup = tk.LabelFrame(cols, text="Создать хранилище", font=("Arial", 11, "bold"), fg="blue", padx=10, pady=10)
        f_backup.grid(row=0, column=0, sticky="nsew", padx=(0, 5))

        # 1. Source folder
        tk.Label(f_backup, text="Исходная папка (что шифруем):").pack(anchor="w")
        ...
        tk.Label(f_backup, text="Папка хранилища (для .enc файлов):").pack(anchor="w")
        ...
        # 3. Manifest folder
        tk.Label(f_backup, text="Папка манифеста (опц., по умолч. /manifest):").pack(anchor="w")
        ...
        # 4. Password
        tk.Label(f_backup, text="Пароль:").pack(anchor="w")
        ...
        tk.Label(f_restore, text="Папка с .enc файлами:").pack(anchor="w")
        ...
        # 3. Where to restore
        tk.Label(f_restore, text="Куда восстановить:").pack(anchor="w")
        ...
        # 4. Password
        tk.Label(f_restore, text="Пароль:").pack(anchor="w")
    ...
    def run_backup(self):
        ...
        if not (src and vault and pwd):
            self.write_log("Заполните основные поля слева!", is_error=True)
            return
        ...
        def worker():
            try:
                self.write_log("--- НАЧАЛО СОЗДАНИЯ МАНИФЕСТА ---")
                ...
                self.write_log(f"Манифест сохранен в:\n{actual_man_path}")
                self.write_log("--- ГОТОВО ---")
            except Exception as e:
                self.write_log(f"КРИТИЧЕСКАЯ ОШИБКА: {e}", is_error=True)
    ...
    def run_restore(self):
        ...
        if not (json_path and vault and out and pwd):
            self.write_log("Заполните все поля справа!", is_error=True)
            return
        ...
        def worker():
            try:
                self.write_log("--- НАЧАЛО ВОССТАНОВЛЕНИЯ ---")
