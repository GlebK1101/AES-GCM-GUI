...
class StreamSingleTab(BaseTab):
    def setup_ui(self):
        ...
        frame_enc = tk.LabelFrame(top_frame, text="Шифрование", font=("Arial", 12, "bold"), fg="#8e44ad", padx=10, pady=10)
        ...
        tk.Button(frame_enc, text="ЗАШИФРОВАТЬ", bg="#e8daef", font=("Arial", 12, "bold"), 
                  command=self.on_encrypt_click).pack(side="bottom", fill='x', pady=5)

        # === RIGHT COLUMN: DECRYPTION (STREAM) ===
        frame_dec = tk.LabelFrame(top_frame, text="Дешифрование", font=("Arial", 12, "bold"), fg="#27ae60", padx=10, pady=10)
        ...
        tk.Button(frame_dec, text="РАСШИФРОВАТЬ", bg="#d5f5e3", font=("Arial", 12, "bold"), 
    ...
    def on_encrypt_click(self):
        ...
        if not path:
            self.write_log("Заполните путь", is_error=True)
            return
        
        if not pwd:
            self.write_log("Заполните пароль", is_error=True)
            return
        ...
        try:
            ...
            self.write_log(f"Потоковое шифрование завершено:\nИсходный: {path}\nПароль: {pwd}\nРезультат: {res}")
            ...
            # If RAM ran out
            self.write_log("КРИТИЧЕСКАЯ ОШИБКА: Не хватает оперативной памяти! Уменьшите Chunk Size в настройках.", is_error=True)
    ...
    def on_decrypt_click(self):
        ...
        try:
            ...
            self.write_log(f"Потоковое дешифрование завершено:\nРезультат: {res}")
        except InvalidTag:
