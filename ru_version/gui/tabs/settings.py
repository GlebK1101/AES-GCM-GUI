...
class SettingsTab(BaseTab):
    def setup_ui(self):
        ...
        # Settings group
        grp_kdf = tk.LabelFrame(center_frame, text="Параметры", 
        ...
        btn_save = tk.Button(grp_kdf, text="Применить настройки", bg="#ddd", command=self.save_settings)
        btn_save.grid(row=6, column=0, columnspan=4, pady=(10, 0), sticky="we") 

        btn_export = tk.Button(grp_kdf, text="Экспорт JSON",  bg="#ddd", command=self.export_config)
        btn_export.grid(row=7, column=0, columnspan=2, pady=(10, 0), sticky="we") 

        btn_import = tk.Button(grp_kdf, text="Импорт JSON",  bg="#ddd", command=self.import_config)
    ...
    def _calc_val(self, val_str):
        ...
        if not val_str.strip():
            raise ValueError("Числовое поле не может быть пустым")
        try:
            return int(eval(val_str, {"__builtins__": None}, {}))
        except Exception:
            raise ValueError(f"Не удалось вычислить значение: {val_str}")
    ...
    def save_settings(self):
        try:
            ...
            if val_min_len > val_max_len:
                raise ValueError(
                    f"Ошибка логики: Min Name Len ({val_min_len}) "
                    f"не может быть больше Max Name Len ({val_max_len})!"
                )
            
            # 3. Extension check
            if not self.enc_ext.get().strip():
                raise ValueError("Поле 'Extension' не может быть пустым")
            ...
            self.write_log(f"Конфигурация применена:\n{formatted}")
            
        except ValueError as e:
            self.write_log(f"Ошибка валидации данных:\n{e}", is_error=True)

    def export_config(self):
        if not self.app.app_config:
            self.write_log("Конфигурация пуста. Сначала нажмите 'Применить'.", is_error=True)
            return
        ...
        if filename:
            try:
                ...
                self.write_log(f"Сохранено в файл:\n{filename}")
            except Exception as e:
                self.write_log(f"Ошибка сохранения: {e}", is_error=True)
    ...
    def import_config(self):
        ...
        if filename:
            try:
                ...
                self.save_settings()
                self.write_log(f"Конфигурация загружена из файла:\n{filename}")
                
            except Exception as e:
                self.write_log(f"Ошибка импорта: {e}", is_error=True)