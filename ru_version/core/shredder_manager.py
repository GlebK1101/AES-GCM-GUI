...
def wipe_file(path: str, passes: int = 1, status_callback=None):
    ...
    with open(path, "rb+") as f:
        for i in range(passes):
            if status_callback:
                status_callback(f"Проход {i+1}/{passes} для: {os.path.basename(path)}")
...
def wipe_directory(dir_path: str, passes: int = 1, status_callback=None):
    ...
    for root, dirs, files in os.walk(dir_path, topdown=False):
        for name in files:
            ...
            try:
                ...
                if status_callback: 
                    status_callback(f"Ошибка удаления файла {name}: {e}", is_error=True)
        ...
        for name in dirs:
            ...
            try:
                ...
                if status_callback:
                    status_callback(f"Не удалось удалить папку {name}: {e}", is_error=True)
    ...
    try:
        ...
        if status_callback: status_callback(f"Ошибка удаления корня {dir_path}: {e}", is_error=True)