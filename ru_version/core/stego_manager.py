...
def hide_data_in_image(cover_path: str, secret_path: str, output_path: str):
    if not os.path.exists(cover_path): raise FileNotFoundError(f"Нет файла: {cover_path}")
    if not os.path.exists(secret_path): raise FileNotFoundError(f"Нет файла: {secret_path}")
...
def extract_data_from_image(stego_path: str, output_dir: str):
    if not os.path.exists(stego_path): raise FileNotFoundError(f"Нет файла: {stego_path}")
    ...
    with open(stego_path, "rb") as f:
        ...
        if f.read() != STEGO_SIGNATURE:
            raise ValueError("Скрытых данных не найдено (нет подписи).")
        ...
        if secret_start_pos < 0:
            raise ValueError("Файл поврежден (некорректные размеры).")
        ...
        if os.path.exists(path_img):
            raise FileExistsError(f"Файл уже существует: {name_img}")
        if os.path.exists(path_sec):
            raise FileExistsError(f"Файл уже существует: {name_sec}")
