...
class App(tk.Tk):
    def __init__(self):
        ...
        self.notebook.add(self.tab_encryption, text='Single File')
        # Поток / Stream
        self.tab_stream = StreamSingleTab(self.notebook, self)
        self.notebook.add(self.tab_stream, text='Stream')
        # Манифест / Manifest
        self.tab_manifest = ManifestTab(self.notebook, self)
        self.notebook.add(self.tab_manifest, text='Manifest')
        # Стеганография / Steganography
        self.tab_stego = StegoTab(self.notebook, self)
        self.notebook.add(self.tab_stego, text='Steganography')
        # Шредер / Shredder
        self.tab_shredder = ShredderTab(self.notebook, self)
        self.notebook.add(self.tab_shredder, text='Shredder')
        # Настройки / Settings
